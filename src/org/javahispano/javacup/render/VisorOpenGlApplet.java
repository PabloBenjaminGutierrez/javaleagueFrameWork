...
public class VisorOpenGlApplet implements Game {
     ...
     *
     */
    public VisorOpenGlApplet(PartidoGuardado partido, int sx, int sy,
     ...
     */
    public VisorOpenGlApplet(PartidoGuardado partido, int sx, int sy,
            boolean fullscreen, PrincipalFrame principal, boolean progreso)
    ...
    public void init(GameContainer gc) throws SlickException {
        ...
        uniformeAlternativoVisita = TacticValidate.useAlternativeColors(
                partido.getDetalleLocal(), partido.getDetalleVisita());
        this.gc = gc;
        pp = new PintaPublicidad("imagenes/logos/pubvert.png",
                "imagenes/logos/pubhor.png", gc.getWidth() / 2,
                gc.getHeight() / 2);
        ...
        gc.setTargetFrameRate(VisorOpenGlApplet.fps);
        gc.setVSync(false);
        pm = new PintaMarcador(partido.getDetalleLocal().getTacticName(),
    ...
    void stop() {
        ...
        if (partido.fueGrabado()) {
            if (JOptionPane.showConfirmDialog(principal,
                    "Desea guardar el partido?", "Guardar Partido",
                    JOptionPane.YES_NO_OPTION) == 0) {
                if (jfc.showSaveDialog(principal) == JFileChooser.APPROVE_OPTION) {
                    try {
                        partido.getPartidoGuardado()
                                .save(jfc.getSelectedFile());
                        if (principal != null) {
                            principal.addGuardadoLocal(new File[]{jfc
                                .getSelectedFile()});
                        }
                    } catch (Exception ex) {
    ...
    public void update(GameContainer gc, int fps) throws SlickException {
        ...
        if (guardado && i.isKeyPressed(Input.KEY_LEFT) && progreso) {
            if (incremento == 0) {
                incremento = -1;
        ...
        if (guardado && i.isKeyPressed(Input.KEY_RIGHT) && progreso) {
            if (incremento == 0) {
                incremento = 1;
            } else if (incremento == 1) {
                incremento = 2;
        ...
        paso = 0;
        if (guardado && i.isKeyPressed(Input.KEY_UP) && progreso) {
            incremento = 0;
            paso = 1;
        }
        if (guardado && i.isKeyPressed(Input.KEY_DOWN) && progreso) {
            incremento = 0;
            paso = -1;
        }
        if (guardado && i.isKeyPressed(Input.KEY_HOME) && progreso) {
            inicio = pg.getTiempo();
        }
        if (guardado && i.isKeyPressed(Input.KEY_END) && progreso) {
            fin = pg.getTiempo();
        }
        if (guardado && i.isKeyPressed(Input.KEY_DELETE) && progreso) {
            if (JOptionPane.showConfirmDialog(null,
                    "Desea eliminar los frames seleccionados?",
                    "Eliminar Frames", JOptionPane.YES_NO_OPTION) == 0) {
                if (inicio < fin) {
        ...
        if (guardado && i.isKeyPressed(Input.KEY_S) && progreso) {
            if (JOptionPane.showConfirmDialog(null, "Desea gardar el partido?",
                    "Guardar Partido", JOptionPane.YES_NO_OPTION) == 0) {
        ...
        vx = (partido.getPosiciones()[2][0].getX() - px)
                / Constants.SEGUIMIENTO_CAMARA;
        vy = (partido.getPosiciones()[2][0].getY() - py)
                / Constants.SEGUIMIENTO_CAMARA;
        ...
        if (partido != null) {
            if (guardado) {
                try {
                    ...
                    logger.error(
                            "Error al establecer tiempo de partido guardo", ex);
        ...
        for (int j = 0; j < 11; j++) {
            ...
            if (dy != 0 || dx != 0) {
                ...
                angulos[j][0] = posPrev[j][0].angle(posActu[0][j]) * 180
                        / Math.PI + 90;
                if (incremento < 0) {
            ...
            if (dy != 0 || dx != 0) {
                ...
                angulos[j][1] = posPrev[j][1].angle(posActu[1][j]) * 180
                        / Math.PI + 90;
                if (incremento < 0) {
        ...
        for (int x = 0; x < 11; x++) {
            for (int y = 0; y < 2; y++) {
                ...
                for (int j = 0; j < angulosAnteriores[0][0].length; j++) {
                    angVisible[x][y] = angVisible[x][y]
                            + angulosAnteriores[x][y][j];
                }
                angVisible[x][y] = angVisible[x][y]
                        / (double) angulosAnteriores[0][0].length;
            }
        }

        if (autoescala) {
            int[] escalas = (Transforma.transform(partido.getPosVisibleBalon(),
                    Constants.centroCampoJuego,
                    -Transforma.transform(px, escala),
                    -Transforma.transform(py, escala), escala));
            escalaAjustada = escala
                    * Math.min(0.7d * sx2 / (double) Math.abs(escalas[0]), 0.7d
                            * sy2 / (double) Math.abs(escalas[1]));
        }
    ...
    public void render(GameContainer gc, Graphics g) throws SlickException {
        ...
        if (autoescala) {
            ...
            if (!noAutoEscalar) {
                ...
                if (partido.getIteracion() < 50) {
                    escalaGradual = escalaGradual + (escala - escalaGradual)
                            * 0.01;
                } else {
                    escalaGradual = escalaGradual + (escala - escalaGradual)
                            * 0.2;
        ...
        if (!partido.esGol() && partido.estanSacando()) {
            ...
            rel = Transforma.transform(pos[2][0], Constants.centroCampoJuego,
                    -Transforma.transform(px, escala) + sx2,
                    -Transforma.transform(py, escala) + sy2, escala);
            g.drawImage(xImage.getScaledCopy((int) zoom, (int) zoom), rel[0]
                    - (int) (zoom / 2), rel[1] - (int) (zoom / 2));

        }
        Position ball = partido.getPosVisibleBalon();
        for (int i = 0; i < 11; i++) {
            rel = Transforma.transform(pos[0][i], Constants.centroCampoJuego,
                    -Transforma.transform(px, escala) + sx2,
                    -Transforma.transform(py, escala) + sy2, escala);
            pjLocal.pintaSombra(i, iteraciones[i][0], angVisible[i][0], escala,
                    rel[0], rel[1], g);
            rel = Transforma.transform(pos[1][i], Constants.centroCampoJuego,
                    -Transforma.transform(px, escala) + sx2,
                    -Transforma.transform(py, escala) + sy2, escala);
            pjVisita.pintaSombra(i, iteraciones[i][1], angVisible[i][1],
                    escala, rel[0], rel[1], g);
        }
        ...
        rel = Transforma.transform(ball, Constants.centroCampoJuego,
                -Transforma.transform(px, escala) + sx2,
                -Transforma.transform(py, escala) + sy2, escala);
        ...
        if (jugador3d) {
            lista.clear();
            for (int i = 0; i < 11; i++) {
                rel = Transforma.transform(pos[0][i],
                        Constants.centroCampoJuego,
                        -Transforma.transform(px, escala) + sx2,
                        -Transforma.transform(py, escala) + sy2, escala);
                lista.add(new Object[]{pjLocal, i, iteraciones[i][0],
                    angVisible[i][0], escala, rel[0], rel[1]});
                rel = Transforma.transform(pos[1][i],
                        Constants.centroCampoJuego,
                        -Transforma.transform(px, escala) + sx2,
                        -Transforma.transform(py, escala) + sy2, escala);
                lista.add(new Object[]{pjVisita, i, iteraciones[i][1],
                    angVisible[i][1], escala, rel[0], rel[1]});
            }
            Object[] tmp1, tmp2;
            ...
            for (Object obj[] : lista) {
                ...
                pj.pintaJugador((Integer) obj[1], (Integer) obj[2],
                        (Double) obj[3], (Double) obj[4], (Integer) obj[5],
                        (Integer) obj[6], g);
            }
            ...
            for (int i = 0; i < 11; i++) {
                rel = Transforma.transform(pos[0][i],
                        ...
                        -Transforma.transform(py, escala) + sy2, escala);
                if (tipoTexto == 3
                        || (pos[0][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
                    pjLocal.pintaNumero(i, rel[0], rel[1], g);
                }
                if (tipoTexto == 4
                        || (pos[0][i].distance(pos[2][0]) < 8 && tipoTexto == 2)) {
                ...
                rel = Transforma.transform(pos[1][i],
                        ...
                        -Transforma.transform(py, escala) + sy2, escala);
                if (tipoTexto == 3
                        || (pos[1][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
                    pjVisita.pintaNumero(i, rel[0], rel[1], g);
                }
                if (tipoTexto == 4
                        || (pos[1][i].distance(pos[2][0]) < 8 && tipoTexto == 2)) {
            ...
            for (int i = 0; i < 11; i++) {
                rel = Transforma.transform(pos[0][i],
                        ...
                        -Transforma.transform(px, escala) + sx2,
                        -Transforma.transform(py, escala) + sy2, escala);
                pjLocal.pintaJugador(i, iteraciones[i][0], angVisible[i][0],
                        escala, rel[0], rel[1], g);
                if (tipoTexto == 3
                        || (pos[0][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
                    pjLocal.pintaNumero(i, rel[0], rel[1], g);
                ...
                rel = Transforma.transform(pos[1][i],
                        ...
                        -Transforma.transform(px, escala) + sx2,
                        -Transforma.transform(py, escala) + sy2, escala);
                pjVisita.pintaJugador(i, iteraciones[i][1], angVisible[i][1],
                        escala, rel[0], rel[1], g);
                if (tipoTexto == 3
                        || (pos[1][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
                    pjVisita.pintaNumero(i, rel[0], rel[1], g);
        ...
        if (marcador) {
            pm.pintaMarcador(partido.getGolesLocal(), partido.getGolesVisita(),
                    partido.getIteracion(), partido.getPosesionBalonLocal(), g);
        }
        if (golIter > 0) {
            double zoom = 1 + 0.05 * (golIter % 3);
            g.drawImage(golImage.getScaledCopy((int) (361d * zoom),
                    (int) (81d * zoom)), sx2 - (int) (180d * zoom), sy2
                    - (int) (40d * zoom));
        }
        ...
        if (guardado && progreso) {
            ...
            int valor = (int) (((double) sy - 41.0) * ((double) pg.getTiempo() / (double) pg
                    .getIterciones()));
            int valorInicio = (int) (((double) sy - 42.0) * ((double) inicio / (double) pg
                    .getIterciones()));
            int valorFin = (int) (((double) sy - 42.0) * ((double) fin / (double) pg
                    .getIterciones()));
            ...
            g.drawLine(sx - 19, sy - 22 - valorInicio, sx - 12, sy - 22
                    - valorInicio);
            g.drawLine(sx - 19, sy - 23 - valorInicio, sx - 18, sy - 23
                    - valorInicio);
            g.drawLine(sx - 13, sy - 23 - valorInicio, sx - 12, sy - 23
                    - valorInicio);
        }
        if (showTexto) {
            if (partido.getIteracion() < 50) {
                g.setColor(Color.black);
                g.drawString(partido.getDetalleVisita().getTacticName()
                        + " (Visita)", sx2 + 11, sy2 + 11);
                g.drawString("vs", sx2 + 41, sy2 + 41);
                g.drawString(partido.getDetalleLocal().getTacticName()
                        + " (Local)", sx2 + 71, sy2 + 71);
                g.setColor(Color.white);
                g.drawString(partido.getDetalleVisita().getTacticName()
                        + " (Visita)", sx2 + 10, sy2 + 10);
        ...
        if (showTexto) {
            if (partido.getIteracion() > Constants.ITERACIONES) {
                ...
                if ((partido.getGolesLocal() > partido.getGolesVisita())
                        || (partido.getGolesLocal() == partido.getGolesVisita() && partido
                        .getPosesionBalonLocal() >= .5d)) {
                    g.drawString(partido.getDetalleLocal().getTacticName(),
                            sx2 + 41, sy2 + 41);
                } else {
                    g.drawString(partido.getDetalleVisita().getTacticName(),
        ...
        /*
         * if (isRain) { g.setColor(lluvia); for (int i = 0; i < 200; i++) {
         * double an = rand.nextDouble() * Math.PI * 2d; double rad =
         * rand.nextDouble() * 1024; int x0 = (int) (sx2 + Math.sin(an) * rad);
         * int y0 = (int) (sy2 + Math.cos(an) * rad); int x1 = (int) (sx2 +
         * Math.sin(an) * rad * 1.1d); int y1 = (int) (sy2 + Math.cos(an) * rad
         * * 1.1d); g.drawLine(x0, y0, x1, y1); } if (rand.nextDouble() < 0.05)
         * { g.setColor(relampago); g.fillRect(0, 0, sx, sy); } isRain =
         * rand.nextDouble() < 0.995; } else { isRain = rand.nextDouble() <
         * 0.005; }
         */
    }
    ...
    public static void setEscala(double escala) {
        VisorOpenGlApplet.escala = escala;
    }
    ...
    public static void setAutoescala(boolean auto) {
        VisorOpenGlApplet.autoescala = auto;
    }
