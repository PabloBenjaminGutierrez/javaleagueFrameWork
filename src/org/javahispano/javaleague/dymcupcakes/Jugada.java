package org.javahispano.javaleague.dymcupcakes;
...
public class Jugada {
    ...
    public int rivalNear(int j) {
        int p = 0;
        for (int i = 0; i < 11; i++) {
            if (rival(i).distance(position(j)) < rival(p).distance(position(j))) {
                p = j;
            }
        }
        return p;
    ...
    public int near(Position pos) {
        ...
        for (int j : involved) {
            ...
            if (pos.distance(position(j)) < pos.distance(position(p))) {
                p = j;
            }
        }
        return p;
    ...
    public Position bestMove(List<Position> alt, int j) {
        ...
        for (Position p : alt) {
            if (best != null) {
                if (evaluateMove(p, j) > evaluateMove(best, j)) {
                    best = p;
                }
            } else {
                best = p;
